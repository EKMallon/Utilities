...
#include "Wire.h"
#define RTCPOWER_PIN 4  //7 for rocket builds,14  //14 =motinos,etc
byte start_address = 0;
byte end_address = 127;

void setup()
{   
  //some of my units use this pin for powering the RTC, need to set high for I2c coms.
  ...
  for( byte addr  = start_address;
    ...
    if (rc == 0) {
      ...
      case 0x18: 
        ...
        break;
      case 0x76: 
        Serial.println("MS5803-CSB high  OR MS561101BA CSB_HIGH "); 
        break;
      case 0x77: 
        Serial.println("MS5803-CSB low OR BMP085"); 
        break;
        case 0x53: 
  ...
  Serial.println("\nStd. Configuration: DS3231: 68, 4kB RTCeeprom: 57, 32kB AT24C256: 50");
  Serial.println("\ndone");  Serial.flush();
}
...
void loop()
{
  // Scan is finished, so we'll just blink the built-in LED
  digitalWrite(5,HIGH); 
  delay(300);
  digitalWrite(5,LOW);  
  delay(300);
